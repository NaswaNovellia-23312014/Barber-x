...
export default function AdminDashboardPage() {
  ...
  /**
   * Menormalisasi format status agar konsisten (Uppercase)
   */
  ...
  /**
   * LOGIKA UTAMA: Filter & Sort Data
   * Diletakkan di sini agar bisa diakses oleh Stats dan CSV Export
   */
  ...
  }, [bookings, activeTab, selectedDate, sortOrder]);
  ...
  const handleExportCSV = () => {
    const headers = [
      "Customer Name", "Phone Number", "Service Name", 
      "Price", "Date", "Time", "Status"
    ];
  ...
  return (
    ...
    <div className="fixed inset-0 z-0 pointer-events-none">
      ...
      <header className="max-w-7xl mx-auto mb-10 px-4 pt-6">
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 bg-white rounded-[32px] shadow-sm border border-slate-100 overflow-hidden flex flex-col">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-100 bg-slate-50/30">
            ...
            {/* Kanan: Toolbar Filter, Sort, & Export */}
            <div className="p-3 sm:pr-6 flex flex-wrap items-center justify-end gap-3">
              
              {/* 1. FILTER TANGGAL */}
              <div className="relative group">
                <div className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 group-hover:text-indigo-500 transition-colors">
                ...
                <input 
                  ...
                  className="pl-9 pr-3 py-2 bg-white border border-slate-200 text-slate-600 rounded-xl text-[11px] font-bold uppercase tracking-wider focus:outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-50 transition-all shadow-sm h-[34px]"
                />
                {selectedDate && (
                   <button 
                     onClick={() => setSelectedDate("")}
                     className="absolute right-8 top-1/2 -translate-y-1/2 text-slate-300 hover:text-red-500"
                     title="Clear Date"
                   >
                     <X size={12} />
                   </button>
              ...
              <button
                ...
                className="flex items-center gap-2 px-3 py-2 bg-white border border-slate-200 text-slate-600 rounded-xl text-[11px] font-bold uppercase tracking-wider hover:bg-slate-50 hover:border-indigo-300 transition-all active:scale-95 shadow-sm h-[34px]"
                title={sortOrder === 'asc' ? "Urutan: Pagi ke Malam" : "Urutan: Malam ke Pagi"}
              >
                <ArrowUpDown size={14} className={sortOrder === 'asc' ? "text-indigo-500" : "text-orange-500"} />
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 bg-emerald-50 border border-emerald-100 text-emerald-700 rounded-xl text-[11px] font-bold uppercase tracking-wider hover:bg-emerald-100 hover:border-emerald-200 transition-all active:scale-95 shadow-sm h-[34px]"
              >
                <Download size={14} />
                Export
              </button>
                ...
                <tbody>
                  {filteredBookings.length === 0 ? (
                    ...
                    filteredBookings.map(b => (
                      <tr key={b.id} className="group bg-white hover:bg-slate-50/80 transition-all duration-300 shadow-sm hover:shadow-md border border-slate-100 rounded-2xl">
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-white p-6 rounded-[32px] shadow-sm border border-slate-100">
            <div className="flex justify-between items-center mb-6">
              <h2 className="font-black text-[10px] uppercase tracking-widest text-slate-400">Services</h2>
              <button 
                onClick={() => { 
            ...
            <div className="space-y-3">
              {services.map(s => (
                <div key={s.id} className="p-4 border border-slate-50 rounded-2xl flex justify-between items-center group hover:border-indigo-100 transition">
                  <div>
                  ...
                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => { setEditingServiceId(s.id); setServiceFormData({...s, description: s.description || ''}); setIsModalOpen(true); }} className="p-2 text-slate-400 hover:text-indigo-600 transition-colors"><Pencil className="w-4 h-4"/></button>
                    <button onClick={async () => { if(confirm('Delete service?')) { await deleteService(s.id); loadData(false); }}} className="p-2 text-slate-400 hover:text-red-600 transition-colors"><Trash2 className="w-4 h-4"/></button>
                  </div>
                </div>
              ))}
            </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-[40px] w-full max-w-md p-8 shadow-2xl">
            <h3 className="text-xl font-black text-slate-900 mb-6 text-center">{editingServiceId ? 'Update Service' : 'New Service'}</h3>
            <form onSubmit={async (e) => {
              ...
              <div>
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-2 mb-1 block">Service Name</label>
                <input required className="w-full p-4 bg-slate-50 border-none rounded-2xl outline-none focus:ring-2 focus:ring-indigo-600 font-bold" value={serviceFormData.name} onChange={e => setServiceFormData({...serviceFormData, name: e.target.value})} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-2 mb-1 block">Price (IDR)</label>
                  <input required type="number" className="w-full p-4 bg-slate-50 border-none rounded-2xl outline-none focus:ring-2 focus:ring-indigo-600 font-bold" value={serviceFormData.price} onChange={e => setServiceFormData({...serviceFormData, price: Number(e.target.value)})} />
                </div>
                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-2 mb-1 block">Duration (Mins)</label>
                  <input required type="number" className="w-full p-4 bg-slate-50 border-none rounded-2xl outline-none focus:ring-2 focus:ring-indigo-600 font-bold" value={serviceFormData.duration} onChange={e => setServiceFormData({...serviceFormData, duration: Number(e.target.value)})} />
                </div>
              </div>
              <div className="flex gap-3 pt-4">
                <button type="button" onClick={() => setIsModalOpen(false)} className="flex-1 py-4 font-black text-xs uppercase tracking-widest text-slate-400 hover:bg-slate-50 rounded-2xl transition">Cancel</button>
